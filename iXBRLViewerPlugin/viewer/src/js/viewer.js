  ...
  return new Promise((resolve, reject) => {
    ...
    (async function () {
      for (var docIndex = 0; docIndex < viewer._iframes.length; docIndex++) {
        await viewer._iv.pluginPromise(
          "preProcessiXBRL",
          viewer._iframes.eq(docIndex).contents().find("body").get(0),
          docIndex
        );
      }
    })()
      .then(() => viewer._iv.setProgress("Preparing document"))
      .then(() => viewer._loadCredentials())
      .then(() => {
      ...
      .then(() => {
        viewer._iframes.each(function (docIndex) {
          viewer._postProcessiXBRL(
            $(this).contents().find("body").get(0),
            docIndex
          );
        });
      });
...
Viewer.prototype._preProcessiXBRL = function (n, docIndex, inHidden) {
  ...
  if (
    ...
    if (!inHidden) {
      ...
      if (node.length == 1 && innerText.length > 0) {
        if (node.css("display") == "none") {
          ...
          const wrapper =
            outerText.substring(0, start) +
            outerText.substring(start + innerText.length);
    ...
    if (name == "NONNUMERIC") {
      ...
      if (
        n.hasAttribute("escape") &&
        n.getAttribute("escape").match(/^(true|1)$/)
      ) {
        ixn.escaped = true;
      }
    }
    ...
    if (elt) {
      ...
      var tr = $("<tr></tr>")
        .append('<td><div title="' + concept + '">' + concept + "</div></td>")
        .append($(elt).wrap("<td></td>").parent());
...
Viewer.prototype._postProcessiXBRLNode = function (container, node, fact) {
  ...
  if (fact) {
    ...
    let sigCount =
      fact.signatures().length + this._report.fullSignatureCredentials().length;
...
Viewer.prototype._selectAdjacentTag = function (offset) {
  ...
  if (current.length == 1) {
    next = elements.eq(
      (elements.index(current.first()) + offset) % elements.length
    );
  } else if (offset > 0) {
    next = elements.first();
  } else {
...
Viewer.prototype._bindHandlers = function () {
  ...
  $(".ixbrl-element", this._contents)
    ...
    .mouseenter(function (e) {
      viewer._mouseEnter($(this));
    })
    .mouseleave(function (e) {
      viewer._mouseLeave($(this));
    });
  $("body", this._contents).click(function (e) {
    var target = $(e.target);
    if (viewer._useFrames || target.closest("#iframe-div").length != 0)
      viewer.selectElement(null);
  });

  $("#iframe-container .zoom-in").click(function () {
    viewer.zoomIn();
  });
  $("#iframe-container .zoom-out").click(function () {
    viewer.zoomOut();
  });
...
Viewer.prototype.showElement = function (e, force) {
  if (e[0] != this._currentShowElement || force) {
    ...
    } else this.scrollIntoViewIfNeeded(e);
  }
};
...
Viewer.prototype.clearHighlighting = function () {
  $("body", this._contents)
    .find(".ixbrl-element")
    .removeClass("ixbrl-selected")
    .removeClass("ixbrl-related")
    .removeClass("ixbrl-linked-highlight");
};
...
Viewer.prototype.elementsForFacts = function (facts) {
  return this.elementsForItemIds(
    $.map(facts, function (f) {
      return f.id;
    })
  );
};
...
Viewer.prototype.highlightItem = function (factId) {
  ...
  this.highlightElements(
    this.elementsForItemIds([factId].concat(continuations))
  );
};
...
Viewer.prototype.highlightTags = function (on, factIds) {
  ...
  if (on) {
    factIds.forEach((factId) => {
      ...
      var elements = viewer.elementsForItemIds(
        [factId].concat(ixn.continuationIds())
      );
...
Viewer.prototype.highlightAllTags = function (on, namespaceGroups) {
  ...
  var viewer = this;
  if (on) {
    $(".ixbrl-element:not(.ixbrl-continuation)", this._contents).each(
      function () {
...
Viewer.prototype.highlightSignedTags = function (on, factIds, signedClass) {
  ...
  if (on) {
    factIds.forEach((factId) => {
      ...
      let signedClasses = elements
        .attr("class")
        .split(/\s+/)
        .filter((cls) => cls.startsWith("ixbrl-signed-"));
...
Viewer.prototype.highlightAllSignedTags = function (
  on,
  namespaceGroups,
  signedClass
) {
  ...
  if (on) {
    ...
    $(".ixbrl-element", this._contents).each(function () {
      ...
      let elements = viewer.elementsForItemIds(
        [factId].concat(ixn.continuationIds())
      );
      let signedClasses = elements
        .attr("class")
...
Viewer.prototype._setTitle = function (docIndex) {
  $("#top-bar .document-title").text(
    $("head title", this._iframes.eq(docIndex).contents()).text()
  );
};
...
Viewer.prototype.selectDocument = function (docIndex) {
  ...
  this._iframes.height(0).eq(docIndex).height("100%");
